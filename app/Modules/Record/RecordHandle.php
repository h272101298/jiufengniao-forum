...
trait RecordHandle
{
    ...
    public function addRecord($user_id,$preDrawIssue,$record){
            ...
            if ($record_int <= $int){
                ...
                if ($res){
                    ...
                    $int=bcsub($int,$record_int,2);
                    DB::table('bet_user')->where('id',$user_id)->update(['cur_integral'=>$int]);
                    return $res;
                }
            }else{
                return false;
    ...
    public function checkRecode($user_id,$preDrawIssue){
        $check=DB::table('bet_user')->where('id',$user_id)->where('preDrawIssue',$preDrawIssue)->first();
        return $check;
